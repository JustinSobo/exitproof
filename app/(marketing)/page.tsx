  ...
  return (
    <div>
      ...
      <section className="relative mx-auto flex min-h-[calc(100vh-5.5rem)] w-full max-w-6xl flex-col justify-center px-6 pb-20 pt-8">
        <div className="ep-shine absolute inset-x-0 top-10 -z-0 mx-auto h-[min(52vh,420px)] max-w-4xl rounded-[2rem] border border-[var(--line)] bg-[radial-gradient(circle_at_30%_20%,rgba(31,196,181,0.18),transparent_55%),linear-gradient(145deg,rgba(15,42,56,0.65),rgba(7,22,31,0.4))]" />
        ...
        <div className="relative z-10 max-w-3xl">
          ...
          <p className="ep-rise-delay-2 mt-4 text-sm text-[var(--fog)]">
            No credit card for the trial gate. Paid plans start at $
            {PLANS.team.priceMonthly}/mo when you need more offboards.
          </p>
        </div>
      ...
      <section id="pricing" className="mx-auto max-w-6xl px-6 py-20">
        ...
        <p className="mt-3 max-w-2xl text-[var(--fog)]">
          Start with <span className="text-white">3 free offboards</span> on the
          trial gate — then subscribe to Team, Growth, or Agency when you need
          volume, retention, or client orgs. No fake “14-day free trial” clock.
        </p>
        <div className="mt-10 grid gap-6 lg:grid-cols-3">
          {paidPlans.map((plan) => (
            <div key={plan.id} className="ep-panel p-6">
              ...
              <ul className="mt-5 space-y-2 text-sm text-[var(--mist)]">
                {plan.features.map((f) => (
                  <li key={f} className="flex gap-2">
                    <span className="text-[var(--teal-bright)]" aria-hidden>
                      ▹
                    </span>
                    {f}
                  </li>
              ...
              <Link
                href="/auth/signup"
                className="ep-btn mt-6 inline-flex rounded-md bg-white/10 px-4 py-2 text-sm font-medium transition-colors hover:bg-white/15"
              >
                Choose {plan.name}
              </Link>
            </div>
